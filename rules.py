...
# The same thing, but used to DECIDE whether a number was typed, so it has to
# be stricter: the number must be followed by a separator or a space. Without
# that, a heading legitimately titled "3D Printing" would look like a typed
# "3" followed by "D Printing".
_TYPED_HEADING_NUMBER_RE = re.compile(r"^\s*\d+(\.\d+)*([.):]|\s)")
...
def check_heading_numbering(model: DocModel, structure, styles) -> list:
    ...
    heading is really auto-numbered. Here we only look at the headings where
    it is False, and there are two quite different reasons that can happen.

    The first is the fault this check exists for: the author typed the
    number into the heading text ("1. Introduction") instead of letting the
    document number it. That is always reported, in every format.

    The second depends on the format, which is why the extractor tells us
    through 'numbering_optional'. In Word, numbering is something the author
    has to switch on, so a heading without it has simply not been given real
    structure - reported. In LaTeX, numbering is the default, so an
    unnumbered heading means the author deliberately wrote \\section* or put
    it past secnumdepth, which is exactly how an Abstract or an
    Acknowledgements section is meant to be written - not reported.
    """
    issues = []
    for b in model.blocks:
        ...
        if _TYPED_HEADING_NUMBER_RE.match(b.text or ""):
            message = (f"Heading '{_preview(b)}' has a typed number "
                       "instead of automatic numbering.")
        elif b.numbering_optional:
            continue  # legitimately unnumbered in this format
        else:
